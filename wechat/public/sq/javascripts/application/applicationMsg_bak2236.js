...
layui.use(['element', 'form', 'jquery'], function() {
	...
	$(function() {
		...
		form.on('submit', function() {
			...
			// 	var selYear = $('#deadline').data('year');
			// 	var selMonth = $('#deadline').data('month') < 10 ? "0" + $('#deadline').data('month') :
			// 		$('#deadline').data('month') + 1;
			// 	var selDay = $('#deadline').data('day') < 10 ? "0" + $('#deadline').data('day') :
			// 		$('#deadline').data('day') + 1;
			// 	strTime = selYear + '-' + selMonth + '-' + selDay;
			// 	console.log(strTime);
			// 	console.log(strTime == '');
            ...
            if(invoiceInfOptionVal) {
            	nbData.invoice_index="";
			}
			console.log(nbData);

			if($("#recipients").text() == "其他") {
				console.log(getRecipientsInfo)
				$.ajax({
					type: "POST",
					url: "/api/save_postinfo",
			...
			if($("#invoiceInfo").text() == "其他") {
                console.log(getInvoiceInfo)
				$.ajax({
					type: "POST",
					url: "/api/save_invoiceinfo",
					data: getInvoiceInfo,
					dataType: "json",
					async: false,
					success: function(data) {
